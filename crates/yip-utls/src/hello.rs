...
//! Task 3, upgraded to full PQ fidelity in Task 9).
//!
...
//! including `X25519MLKEM768` (`4588`) — real Chrome 150 sends it as its
//! PRIMARY `key_share` entry, and this crate now carries a genuine ML-KEM-768
//! encapsulation key (generated by the caller, e.g. [`crate::stream::connect`],
//! and threaded in via [`ClientHelloParams::key_share_mlkem_ek`]) rather than
//! the placeholder-free/dropped group Task 8 shipped — see
//! [`key_share_body`]'s and [`supported_groups_body`]'s doc comments for why
//! Task 8 had to drop it and how Task 9 restores it. Only the *order* of the
//! real extensions is randomized per connection via a Fisher–Yates shuffle
//! seeded from the caller's [`RandomSource`] — the fingerprint reference
//! documents three real Chrome captures with three different JA3 hashes
//! (order-sensitive) but one stable JA4 (order-insensitive, it sorts): modern
//! Chrome/BoringSSL permutes its ClientHello extension order every
//! connection, keeping one GREASE extension first and one last. A FIXED
//! extension order here would make yip MORE fingerprintable than real
//! Chrome — defeating the point of this crafter.
...
/// Byte length of a real ML-KEM-768 encapsulation key (FIPS 203 §7.1,
/// `EncapsulationKeySize` for the category-3 parameter set).
...
pub struct ClientHelloParams {
    ...
    /// The caller's real ML-KEM-768 encapsulation key, exactly
    /// [`MLKEM768_EK_LEN`] (1184) bytes (FIPS 203 §7.1). `craft` does not
    /// generate this itself — ML-KEM keygen needs a CSPRNG and this module
    /// only ever transforms already-supplied bytes — so the caller (e.g.
    /// [`crate::stream::connect`]) generates the keypair, keeps the
    /// decapsulation key for completing the hybrid exchange once the server's
    /// `ServerHello` arrives, and passes the encapsulation key's encoded
    /// bytes in here.
    pub key_share_mlkem_ek: Vec<u8>,
...
/// `supported_groups` (RFC 8446 §4.2.7): GREASE, `X25519MLKEM768` (`4588`),
/// then the three classical curves, byte-faithful to real Chrome-150's
/// `GREASE,4588,29,23,24` order. Task 8 shipped without `4588` here because
/// this crate couldn't back it with a real key share (see
/// [`key_share_body`]'s doc comment for that history) — Task 9 restores it
/// now that [`key_share_body`] carries a genuine ML-KEM-768 key.
fn supported_groups_body(grease_value: u16) -> Vec<u8> {
    let mut w = HelloWriter::new();
    w.u16_prefixed(|w| {
        w.u16(grease_value);
        w.u16(GROUP_X25519MLKEM768);
        w.u16(GROUP_X25519);
...
/// then `X25519MLKEM768` (`4588`) carrying the caller's real ML-KEM-768
/// encapsulation key concatenated with the caller's X25519 public key (ML-KEM
/// first, then X25519 — draft-kwiatkowski-tls-ecdhe-mlkem's hybrid layout,
/// confirmed against
/// `docs/superpowers/specs/reality-2-chrome150-fingerprint.txt`'s captured
/// 1216-byte entry: `1184 + 32 = 1216`), then plain X25519 carrying the same
/// public key again on its own.
///
/// REALITY.2 Task 8 had to drop the `4588` entry entirely: unlike GREASE (an
/// intentionally-unknown codepoint every RFC 8701-aware server ignores),
...
/// connection, drawing `decode_error`/`illegal_parameter`. Task 9 fixes this
/// at the root by having the caller (see [`crate::stream::connect`]) generate
/// a genuine ML-KEM-768 keypair via the `ml-kem` crate and pass the real
/// encapsulation key in as [`ClientHelloParams::key_share_mlkem_ek`], so the
/// bytes written here always pass canonical-encoding validation.
fn key_share_body(
    ...
    debug_assert_eq!(
        params.key_share_mlkem_ek.len(),
        MLKEM768_EK_LEN,
        "caller contract: key_share_mlkem_ek must be a real, FIPS-203-encoded \
    ...
    w.u16_prefixed(|w| {
        ...
        w.u16(GROUP_X25519MLKEM768);
        w.u16_prefixed(|w| {
            w.bytes(&params.key_share_mlkem_ek);
            w.bytes(&params.key_share_x25519_pub);
        });
...
mod tests {
    ...
    fn test_params() -> ClientHelloParams {
        ClientHelloParams {
            ...
            key_share_mlkem_ek: vec![0xab; MLKEM768_EK_LEN],
    ...
    fn craft_produces_well_formed_chrome150_hello() {
        ...
        // supported_groups (ext 10): GREASE,4588,29,23,24 — byte-faithful to
        // real Chrome-150 now that key_share can back 4588 with a real key
        // (Task 9).
        ...
        // key_share (ext 51): GREASE(1B), 4588(1216B = mlkem ek(1184) ‖
        // x25519 pub(32)), 29(our 32B pub) — see key_share_body's doc
        // comment.
        let ks_body = &parsed
            .extensions
            .iter()
        ...
        assert_eq!(entries[1].0, GROUP_X25519MLKEM768);
        assert_eq!(entries[1].1.len(), MLKEM768_EK_LEN + 32);
        assert_eq!(
            &entries[1].1[..MLKEM768_EK_LEN],
            &params.key_share_mlkem_ek[..]
        );
        assert_eq!(
            &entries[1].1[MLKEM768_EK_LEN..],
            &params.key_share_x25519_pub[..]
        );
