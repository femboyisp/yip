...
Behavior:
- **Peer is `Handshaking`:** mutate the existing `HandshakingState` in place — set `target = new_target`, set the peer's `relay = via_relay` — leaving `hs` and `init_pkt` untouched (the ephemeral is preserved). Emit the existing `init_pkt`: relay-wrapped via `relay_wrap(idx, init_pkt.clone())` when `via_relay` (a `None` skips this send; the retransmit arm retries), else `EgressDatagram { fate: 0, dst: new_target, bytes: init_pkt.clone() }`. Do **not** reset `started_ms` (the 90 s give-up clock keeps running across re-targets — a re-target does not buy a fresh 90 s).
